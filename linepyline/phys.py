...
#Make a dictionary of all the gases
#
...
for ob in dir():
    ...
    if isGas:
        command = f'gases["{ob}"] = {ob}'
        exec(command)
        
# call update method on each gas to compute R
for _ in gases: gases[_].update()

#----------------Radiation related functions-------------

#Planck function (of frequency)
def B_frequency(nu,T):
    u = min(h*nu/(k*T),500.) #To prevent overflow
    return (2.*h*nu**3/c**2)/(np.exp(u)-1.)

#RCABA ----------Humidity unit conversions---------------
#       all assume water vapor mixed with dry Earth air

def epsilon(mol_name, mol_weight_other=None):
    '''
    return ratio of molecular weight of mol_name to
    molecular_weight of other gas (dry air by default)
    '''
    if mol_weight_other is None:
        eps = gases[mol_name].MolecularWeight / gases['air'].MolecularWeight
    else:
        eps = gases[mol_name].MolecularWeight / mol_weight_other
    return eps
...
def q_to_f(q):
    ...
    return:
    molar density (number fraction) of water
    '''
    eps = epsilon('H2O')
    return q/(eps + (1 - eps)*q)

def f_to_q(f):
    '''
    given:
    molar density (number fraction) water
    return:
    specific humidity (kg water/kg moist air)
    '''
    eps = epsilon('H2O')
    return eps*f/(1 - (1 - eps)*f)

def q_to_pp(p, q):
    '''
    given:
    total pressure p (Pa)
    specific humidity q (kg water/kg moist air) 
    return:
...
def pp_to_q(p, pp):
    ...
    given:
    total pressure p (Pa)
    partial pressure pp (Pa) of water
    specific humidity (kg water/kg moist air)
    '''
    f = pp/p
    return f_to_q(f)

def q_to_RH(p, T, q, formula='simple', RH_definition='e/esat'):
    '''
    given:
    total pressure p (Pa)
    temperature T (K)
    ...
    relative humidity
    '''
    esat = satvp(T, formula)
    if RH_definition == 'e/esat':
        e = q_to_pp(p, q)
        RH = e/esat
    else:
        # mass mixing ratio
        w = q/(1 - q) 
        wsat = epsilon('H2O')*esat/(p - esat)
        RH = w/wsat
    return RH

def RH_to_q(p, T, RH, formula='simple', RH_definition='e/esat'):
    '''
    given:
    total pressure p (Pa)
    temperature T (K)
...
# generic call, specify formula through keyword
def satvp(T, formula='simple'):
    if formula == 'simplest':
        return satvpw_simplest(T)
    elif formula == 'simple':
        return satvpw_simple(T)
    elif formula == 'liquid':
        return satvpw(T)
    elif formula == 'ice':
        return satvpi(T)
...
# Sat vapor pressure over liquid water, assuming constant latent heat
def satvpw_simplest(T):
    T0 = 273.15
    e0 = 611. # Pa
    Rv = gases['H2O'].R
    Lv = gases['H2O'].L_vaporization_TriplePoint
    return e0 * np.exp( -Lv/Rv*(1./T - 1./T0) )

# Sat vapor pressure over liquid water, taking temperature dependence of
# latent heat into account
def satvpw_simple(T):
    T0 = 273.15
    e0 = 611. # Pa
    return e0 * np.exp( 6808.*(1./T0 - 1./T) - 5.09*np.log(T/T0) )
...
#ice saturation as the temperature falls below 0C.
   if hasattr(T, '__len__'):  
       T = np.array(T)
       T_C = T - 273.16
       satvp = np.where(T_C < -20., satvpi(T), satvpw(T))
       satvp = np.where( (T_C >= -20.)&(T_C <= 0.),
                         0.05*(273.16-T)*satvpi(T) + 0.05*(T-253.16)*satvpw(T),
                         satvp)
       return satvp
   else:
       if ((T-273.16) <  -20.):
